...
- **所属里程碑**：[M5 — 加固与扩展](../roadmap.md#m5)（热重载 / 可见性在 [M8](../roadmap.md#m8)）
...
- MCP 资源与提示词（[M8](../roadmap.md#m8)）
- OAuth / 远程鉴权
- 配置热重载与设置页内编辑（M8：设置页能看到推荐服务器，安装可先复制命令）
- 按会话 kind 细分可见性（M8）
- **不**再装一份 pi 的 MCP 扩展——两套会抢工具名、审批对不上。浏览器 / 搜索优先走 [扩展与技能](扩展与技能.md) 的默认集；用户仍可在 `mcp.json` 自行加 MCP 服务器。
...
| 2026-08-17 | M8 设计：本模块补 resources / 热重载 / 可见性；与 pi 扩展默认集分工写清 |
