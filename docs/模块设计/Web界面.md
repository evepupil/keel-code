...
- **所属里程碑**：[M1 — 最小聊天](../roadmap.md#m1) → [M2 — 对话树与名册](../roadmap.md#m2) → [M4 — 设计确认与看板](../roadmap.md#m4)（日常驾驶在 [M9](../roadmap.md#m9)，并行与安全网在 [M10](../roadmap.md#m10)）
- **当前状态**：已完成（M1–M5 各功能面已齐，M7 前端整体重构与多工作区验收完成）
- **最近更新**：2026-08-18
...
- `features/chat/ChatView.tsx`：头部只留标题 + 一行职责、右上上下文抽屉开关；空对话居中字标 + 输入卡；有消息后时间线贴底、输入卡贴底。子 agent 只读，头部面包屑 +「返回对话」，无输入框。`MessageItem.tsx`：用户气泡灰底圆角（右下小角）、assistant markdown + 悬停信息行（复制 / 时刻·用时·速度·输出量）、思考行与工具行都是 30px 边框行（点开看详情），流式光标。`features/composer/`：`Composer`（卡片、权限 chip、模型 + 思考档菜单、上下文环、发送 / 停止——停止点击后本地立即退出流式态再等服务端校准；输入框 ArrowUp/Down 调已发送历史（`input-history.ts` 纯函数，localStorage 存最近 100 条，输入非空且未在浏览态时不抢键））、`PullBar`（看板 / 子 agent / 任务，互斥上拉）、`stats.ts`（轮 / 步 / token / 缓存命中）。`features/context/ContextDrawer.tsx`：可关的右侧抽屉，名册记录 + token，无费用。
- `features/settings/SettingsDialog.tsx`：全局设置弹窗（`Dialog size=lg`，左侧竖 tab：模型 / 能力档 / 项目 / MCP / 通用；`appStore.openSettings(tab)` 从任何地方打开，`#/settings` 深链也打开它）。`tabs/ModelsTab.tsx`：只列用户加过的提供方（行：名称 / 内置|自定义 / 状态点 / 编辑 / 删除）；底部「添加提供方」「添加自定义提供方」点开后换成灰底卡片（内置下拉选 pi 目录 + 密钥 + 可展开自定义设置；自定义按参考图填 ID / 名称 / 地址 / 协议 / 密钥；编辑预填同一张卡，ID 锁住）；模型目录：获取后勾选要用的（拉下来默认不勾，手加默认勾），选择器只显示勾上的。`tabs/ProjectTab.tsx` 包 `ProjectConfig`；`tabs/McpTab.tsx`；`tabs/GeneralTab.tsx`：主题（跟随系统 / 亮 / 暗）。`ModelTiers.tsx`：三档各选「供应商 + 模型」（模型来自已添加提供方，不标价格），记为该档首选；各类对话默认档；「锁定具体模型…」收起。
...
- 多工作区验收（2026-08-18，Node 集成）：`keel web` 复用同一个后台实例注册第二个项目；两个工作区的项目路由、会话目录和 WebSocket 事件按 `workspaceId` 隔离。
- 冻结闭环验收（2026-08-18，Playwright + Edge）：冻结卡片渲染、文档路径进入编辑器、冻结徽标与冻结行显示均通过，页面布局无重叠。
...
- **日常驾驶**（[M9](../roadmap.md#m9)）：回形针可用（粘贴 / 拖入 / 选文件）；`@` 弹出文件和对话；计划 chip + 计划卡；项目设置里的 `ask` / `deny` 列表和只读危险清单。
- **并行与安全网**（[M10](../roadmap.md#m10)）：新建对话勾隔离副本；消息「退回这一轮」；用量环弹出分项和「现在压缩」；任务面板接后台活；通用设置通知四开关。

## 模型目录刷新规则

获取模型列表时，设置页把当前提供方的 `kind` 传给服务端。自定义 Anthropic 兼容渠道不携带官方 `anthropic-version` 请求头，避免网关返回 Fable 目录；已有 `models.json` headers 由引擎继续带上。

设置页点击“获取模型列表”后，以远端返回的模型 ID 同步目录：仍在远端的条目保留原有勾选和模型元数据；新条目默认不勾选；远端已不存在且当前未勾选的旧条目会清掉。这样可以清理历史拉取残留，同时保留手动添加或明确启用的模型。
...
| 2026-08-17 | 模型目录改勾选：拉下来默认不勾，手加默认勾；选择器 / 对话框只认勾过的，不再冒未添加的内置 Claude |
| 2026-08-17 | M9 / M10 设计：回形针、`@`、计划卡、用量环弹出、退回这一轮、通知开关 |
| 2026-08-18 | M7 收口：补单实例复用、双工作区隔离与 WebSocket 事件标记集成验收 |
| 2026-08-18 | 修复设置页获取模型列表只追加不清理的问题：同步远端目录，清理未勾选的过期条目并保留手动/已启用模型 |
| 2026-08-18 | M4 收口：冻结卡片、文档入口与编辑器冻结徽标浏览器验收 |
