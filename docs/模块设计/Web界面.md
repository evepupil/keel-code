...
- `features/chat/ChatView.tsx`：头部只留标题 + 一行职责、右上上下文抽屉开关；空对话居中字标 + 输入卡；有消息后时间线贴底、输入卡贴底。子 agent 只读，头部面包屑 +「返回对话」，无输入框。`MessageItem.tsx`：用户气泡灰底圆角、assistant markdown、思考折叠、工具行。`features/composer/`：`Composer`（卡片、权限 chip、模型 + 思考档菜单、上下文环、发送 / 停止）、`PullBar`（看板 / 子 agent / 任务，互斥上拉）、`stats.ts`（轮 / 步 / token / 缓存命中）。`features/context/ContextDrawer.tsx`：可关的右侧抽屉，名册记录 + token，无费用。
...
- Trajectory 回放视图
- 任务清单工具与 `keel/turn` 条目（输入框上拉「任务」、消息悬停用时）
