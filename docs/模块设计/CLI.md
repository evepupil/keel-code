...
`keel-code` 与 `@keel-code/*`（web、testkit 除外）均为可发布包；`prepack` 跑 `scripts/copy-web.mjs` 把 `packages/web/dist` 复制到 `packages/cli/web/`（发布产物含 Web 工作台）。8 个 `@keel-code/*` 运行包已发布 `0.1.0`；由于 npm 不允许复用已撤回的 `keel-code@0.1.0`、`0.1.1`，CLI 使用 `0.1.2` 发布。发布前先执行 `npm login --registry=https://registry.npmjs.org/ --auth-type=web` 并确认 `npm whoami` 输出 npm 用户，再用 `pnpm --filter keel-code publish --no-git-checks`，`workspace:*` 会被替换为已发布版本号。开发期用 `pnpm keel <命令>`（根脚本）。
...
| 2026-08-19 | M6：8 个运行包已发布 `0.1.0`；CLI 因 npm 版本不可复用改为 `0.1.2`，待单独发布 |
