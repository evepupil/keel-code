...
| [M5](#m5) | 加固与扩展：审批三档、MCP 客户端、压缩策略、`keel run` 无头、文档修剪 job、workflow 编排（脚本化多 agent） | 进行中 | M4 | [引擎](模块设计/引擎.md) · [文档管理](模块设计/文档管理.md) · [CLI](模块设计/CLI.md) · [MCP](模块设计/MCP.md) | 审批三档 ✅ / `keel run` ✅ / 文档修剪 job ✅ / MCP 客户端 ✅ / workflow 编排 ✅（2026-08-17）；压缩策略沿用 pi 默认自动压缩（真实使用后再调）；无头回归稳定；真实项目跑通完整流程 |
