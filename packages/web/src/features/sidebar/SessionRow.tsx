...
/** 与项目名对齐：左侧 padding + 文件夹图标宽 + gap */
const TITLE_PAD = "pl-[2.125rem]";
  ...
  return (
    <HoverCard>
      <HoverCardTrigger asChild>
        <div
          className={cn(
            "group flex w-full items-center rounded-md text-left text-sm font-medium",
        ...
        >
          <button
            ...
            className={cn("flex min-w-0 flex-1 items-center gap-1.5 py-1.5 pr-1", TITLE_PAD)}
          >
            <span className="min-w-0 flex-1 truncate text-left">{item.meta.title}</span>
          ...
          <span className="relative mr-1 h-[22px] w-10 shrink-0">
            <span className="absolute inset-0 flex items-center justify-end text-[11px] font-normal text-ink-faint group-hover:invisible">
              {formatRelative(item.lastActiveAt)}
            </span>
            {!main ? (
              <span className="invisible absolute inset-0 flex items-center justify-end group-hover:visible">
                <IconButton
                  size="xs"
